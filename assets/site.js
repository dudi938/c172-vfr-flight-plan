/* ═══════════════════════════════════════════════════════
   VFR Flight Plan — Premium Interactions
   ═══════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function render(q) {
    ...
    var hits = INDEX.filter(function (e) {
      return e.t.toLowerCase().indexOf(q) !== -1 ||
             e.k.indexOf(q) !== -1 ||
             e.r.toLowerCase().indexOf(q) !== -1;
    }).slice(0, 20);
    if (!hits.length) {
  ...
  window.addEventListener('load', function () { setTimeout(flashTarget, 300); });
  ...
  function ensureLightbox() {
    ...
    lightboxImg.alt = 'תמונה';
    lightboxImg.id = 'lbImg';
  ...
  document.querySelectorAll('.img-grid img').forEach(function (img) {
    ...
    img.addEventListener('error', function () {
      if (img.src.indexOf('placehold.co') === -1) {
        img.src = 'https://placehold.co/1400x800/f1f5f9/4a5568?text=Image+Unavailable';
      }
    }, { once: true });
  });
  ...
  var revealElements = document.querySelectorAll('.card, .trip, .ov-card, .airport');
  ...
  if (window.L && document.querySelector('.map')) {
    var routes = {
      ...
      mapD1: [{n:'KFRG',c:[40.7288,-73.4134]},{n:'26N',c:[39.2634,-74.6075]},{n:'KFRG',c:[40.7288,-73.4134]}],
      mapD2: [{n:'KFRG',c:[40.7288,-73.4134]},{n:'KLNS',c:[40.1217,-76.2960]},{n:'N40',c:[40.5659,-74.9793]},{n:'KFRG',c:[40.7288,-73.4134]}],
      mapD3: [{n:'KCDW',c:[40.8753,-74.2814]},{n:'VZ',c:[40.6066,-74.0447]},{n:'Statue',c:[40.6892,-74.0445]},{n:'Intrepid',c:[40.7645,-73.9996]},{n:'GWB',c:[40.8517,-73.9527]},{n:'Alpine',c:[40.9557,-73.9180]},{n:'KCDW',c:[40.8753,-74.2814]}],
      mapD4: [{n:'KFRG',c:[40.7288,-73.4134]},{n:'KBID',c:[41.1681,-71.5776]},{n:'KFRG',c:[40.7288,-73.4134]}],
      mapD5: [{n:'KGAI',c:[39.1686,-77.1659]},{n:'KTGI',c:[37.8250,-75.9920]},{n:'KGAI',c:[39.1686,-77.1659]}]
    };
    ...
    Object.entries(routes).forEach(function (entry) {
      ...
      var line = L.polyline(pts.map(function (p) { return p.c; }), {
        color: '#3B82F6',
        weight: 4,
        opacity: 0.85
      }).addTo(map);
    ...
    window.addEventListener('resize', function () {
      ...
      mapResizeTimer = setTimeout(function () {
        Object.keys(renderedMaps).forEach(function (id) {
          ...
          m.invalidateSize();
          m.fitBounds(L.polyline(routes[id].map(function (p) { return p.c; })).getBounds(), { padding: [24, 24] });
        });
      }, 200);
    });
    ...
    shell.className = 'map-popup-shell';
    ...
    function drawPopupRoute(mapId) {
      ...
      popupLeafletMap.fitBounds(line.getBounds(), { padding: [30, 30] });
    }

    window.openMapPopup = function (mapId) {
      mapPopup.classList.add('open');
      if (!popupLeafletMap) {
        popupLeafletMap = L.map(mapDiv, { scrollWheelZoom: true });
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
          maxZoom: 16,
          attribution: '&copy; OpenStreetMap'
        }).addTo(popupLeafletMap);
      }
      setTimeout(function () {
        popupLeafletMap.invalidateSize();
        drawPopupRoute(mapId);
      }, 50);
    };
